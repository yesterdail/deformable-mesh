...
char* SIVANfactor[]   = {"taucs.factor.LLT=true", NULL};
char* SIVANfactorLU[] = {"taucs.factor.LU=true", NULL};
void* SIVANopt_arg[]  = { NULL }; 
char* SIVANsolve []   = {"taucs.factor=false", NULL};
...
taucs_ccs_matrix *MatrixTranspose(const taucs_ccs_matrix *mat) {
	...
	for (int r = 0; r < mat->m; ++r) {
		...
		for (unsigned int j = 0; j < rows[r].size(); ++j) {
					 ...
					 taucsType * b)
{
	if (matA->flags & TAUCS_SYMMETRIC) {
		...
		for (int col = 0; col < matA->n; ++col) {
			// going over column col of matA, multiplying
			// it by x[col] and setting the appropriate values
			// of vector b; also mirroring the other triangle
			for (int p = matA->colptr[col]; p < matA->colptr[col+1]; ++p) {
				b[matA->rowind[p]] += x[col]*matA->taucs_values[p];
				// mirroring
				if (matA->rowind[p] != col)
					b[col] += x[matA->rowind[p]]*matA->taucs_values[p];
			}
		}
	}
	else {
		MulNonSymmMatrixVector(matA, x, b);
	}
}
